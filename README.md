# adventOfCode2021

Solutions for [Advent of Code 2021](https://adventofcode.com/2021)

Things I needed to know to do these with python

# Language reference
- [comparisons](https://docs.python.org/3.9/reference/expressions.html#comparisons) 
- [boolean logic](https://docs.python.org/3.9/reference/expressions.html#boolean-operations)
- [functions](https://docs.python.org/3.9/reference/compound_stmts.html#function-definitions)
- [if](https://docs.python.org/3.9/reference/compound_stmts.html#the-if-statement)
- [for](https://docs.python.org/3.9/reference/compound_stmts.html#the-for-statement)
- [with](https://docs.python.org/3.9/reference/compound_stmts.html#the-with-statement)
- [pass](https://docs.python.org/3.9/reference/simple_stmts.html#the-pass-statement)

# Built-in functions
- [all](https://docs.python.org/3/library/functions.html#all)
- [int](https://docs.python.org/3/library/functions.html#int)
- [open](https://docs.python.org/3/library/functions.html#open)
- [print](https://docs.python.org/3/library/functions.html#print)
- [str](https://docs.python.org/3/library/functions.html#func-str)
- [map](https://docs.python.org/3/library/functions.html#map)

# Library reference
- [copy](https://docs.python.org/3/library/copy.html)
- [dict](https://docs.python.org/3.9/library/stdtypes.html#mapping-types-dict)
- [itertools](https://docs.python.org/3.9/library/itertools.html)
- [list](https://docs.python.org/3.9/library/stdtypes.html#list)
- [logging](https://docs.python.org/3.9/library/logging.html)
- [string.join](https://docs.python.org/3/library/stdtypes.html#str.join)
- [string.split](https://docs.python.org/3/library/stdtypes.html#str.split)
- [string.rstrip](https://docs.python.org/3/library/stdtypes.html#str.rstrip)
